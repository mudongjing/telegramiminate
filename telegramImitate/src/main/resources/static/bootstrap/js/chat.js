...
function userLogin() {
    ...
    if (userNick) {
        if (!window.WebSocket) {
            ...
            console.log("login window.WebSocket"+JSON.stringify(window.WebSocket));
        }
        if (window.WebSocket) {
            window.socket = new WebSocket("ws://127.0.0.1:9688/websocket");
            window.socket.onmessage = function (event) {
                console.log("user login event:"+JSON.stringify(event));
